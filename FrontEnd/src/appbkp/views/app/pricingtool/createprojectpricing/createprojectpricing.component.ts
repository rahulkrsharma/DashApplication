...
export class CreateprojectpricingComponent implements OnInit {
  ...
  savePricingDetails(){
    ...
    this.ProjectPricingDetails.ExpenseQuote = this.expenseQuote;
    if(this.priceQuoteContractor.length > 0){
...
if(this.priceQuote[rowIndex]['WorkingDays'] > 0){
  ...
  this.priceQuote[rowIndex]['Subtotal'] = (+this.priceQuote[rowIndex]['WorkingDays'] * this.priceQuote[rowIndex]['AppliedRate']);
...
if(this.priceQuoteContractor[rowIndex]['WorkingDays'] > 0){
  this.priceQuoteContractor[rowIndex]['Subtotal'] = (+this.priceQuoteContractor[rowIndex]['WorkingDays'] * this.priceQuoteContractor[rowIndex]['ClientRate']);
...
updateAssetsValue(event, cell, rowIndex) {
  ...
  // if(this.priceQuote[rowIndex]['Capacity'] >= 10){
    this.CalculateFteRatePerWeek(this.priceQuote[rowIndex]['Capacity'],rowIndex);
  //}
...
updateContractorValue(event, cell, rowIndex) {
  ...
  if(this.priceQuoteContractor[rowIndex]['ClientRate']>0 && this.priceQuoteContractor[rowIndex]['TotalCost']>0 ){
    ...
    this.priceQuote[rowIndex]['TotalCost'] = Number(this.priceQuote[rowIndex]['TotalCost']);
  ...
  if(this.priceQuoteContractor[rowIndex]['Capacity'] >= 10){
    this.CalculateContractorRatePerWeek(this.priceQuoteContractor[rowIndex]['Capacity'],rowIndex);
  }
...
validateFields(projectDetails : ProjectDetails){
  ...
  if(this.projectDetails.ServiceType == ''){
