...
@Component({
  selector: 'app-projectpricing',
  templateUrl: './projectpricing.component.html',
  styleUrls: ['./projectpricing.component.scss']
})
export class ProjectpricingComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.projectPricingService.getProjects("CreatedDate","").subscribe(
      result=>{
  ...
  loadData(pageSize: number = 10, currentPage: number = 1, search: string = '', orderBy: string = '') {
    ...
    this.search = search;
    this.orderBy = orderBy == '' ? "CreatedDate":orderBy;

    this.SpinnerService.show();
    this.projectPricingService.getProjects(this.orderBy,search).subscribe(
      result=>{
        this.projectsList = result;
        this.temp = [...this.projectsList];
        this.SpinnerService.hide();
  ...
  deletePricing(index:number,projectName:string){
    this.SpinnerService.show();
    this.projectPricingService.deletePricing(projectName).subscribe(
      result=>{
        this.projectsList = result;
        this.temp = [...this.projectsList];
        this.SpinnerService.hide();
