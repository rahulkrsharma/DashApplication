using Perm_Api.Models;
using System;
using System.Collections.Generic;
using System.Data;
using System.Data.SqlClient;
using System.IO;
using System.Security.Cryptography;
using System.Text;

namespace Perm_Api.Data
{
    public class UserAuthRepository : IUserAuthRepository
    {
        //SqlConnection con = new SqlConnection(@"Data Source = DESKTOP-6NDADEV\SQLEXPRESS01; Initial Catalog = PermDB;Integrated Security=SSPI;");
        SqlConnection con = new SqlConnection(@"Data Source = dash.crbxbf5fbyts.us-east-1.rds.amazonaws.com,1433; Initial Catalog = Dashdb;User Id=admin;Password=Bico12345;");
        public (int, int) Register(UserDetails user)
        {
            Random r = new Random();
            int verificationCode = r.Next(1000000);
            con.Open();
            try
            {
                int result;
                SqlCommand cmd = new SqlCommand("spRegisterUser", con);
                cmd.CommandType = CommandType.StoredProcedure;
                cmd.Parameters.AddWithValue("@UserName", user.displayName);
                cmd.Parameters.AddWithValue("@UserPassword", user.password);
                cmd.Parameters.AddWithValue("@UserEmail", user.email);
                cmd.Parameters.AddWithValue("@UserCountry", user.Location);
                cmd.Parameters.AddWithValue("@Vercode", verificationCode);
                cmd.Parameters.AddWithValue("@VercodeExpirationDttm", DateTimeOffset.Now.AddMinutes(15));
                cmd.Parameters.Add("@Return", SqlDbType.Int);
                cmd.Parameters["@Return"].Direction = ParameterDirection.Output;
                cmd.ExecuteNonQuery();
                result = (int)cmd.Parameters["@Return"].Value;

                return (result, verificationCode);
            }
            catch (Exception ex)
            {
                return (0, 0);
            }
            finally
            {
                con.Close();
            }
        }


        public SignInResponse SignIn(Credentials credentials)
        {
            con.Open();
            try
            {
                int result;
                var response = new SignInResponse();
                SqlCommand cmd = new SqlCommand("spSigninUser", con);
                cmd.CommandType = CommandType.StoredProcedure;
                cmd.Parameters.AddWithValue("@UserEmail", credentials.email);
                cmd.Parameters.AddWithValue("@UserPassword", credentials.password);
                cmd.Parameters.Add("@Return", SqlDbType.Int);
                cmd.Parameters["@Return"].Direction = ParameterDirection.Output;
                cmd.Parameters.Add("@UserID", SqlDbType.Int);
                cmd.Parameters["@UserID"].Direction = ParameterDirection.Output;
                cmd.Parameters.Add("@Level", SqlDbType.NVarChar,40);
                cmd.Parameters["@Level"].Direction = ParameterDirection.Output;
                cmd.Parameters.Add("@UserName", SqlDbType.NVarChar, 40);
                cmd.Parameters["@UserName"].Direction = ParameterDirection.Output;
                cmd.Parameters.Add("@IsAdmin", SqlDbType.NVarChar, 40);
                cmd.Parameters["@IsAdmin"].Direction = ParameterDirection.Output;
                cmd.ExecuteNonQuery();
                string resultValue = cmd.Parameters["@Return"].Value.ToString();
                response.code = Convert.ToInt32(resultValue);
                response.UserID = Convert.ToInt32(cmd.Parameters["@UserID"].Value);
                response.Level = cmd.Parameters["@Level"].Value.ToString();
                response.UserName = cmd.Parameters["@UserName"].Value.ToString();
                response.IsAdmin = cmd.Parameters["@IsAdmin"].Value.ToString();
                if (string.IsNullOrEmpty(resultValue) && resultValue != "4")
                {
                    result = 3;
                }
                else
                {
                    result = Convert.ToInt32(resultValue);
                }
                return response;
            }
            catch (Exception ex)
            {
                Console.WriteLine("Exception is : " + ex.Message);
                return null;
            }
            finally
            {
                con.Close();
            }
        }

        public int ResetPassword(ResetPassword credentials)
        {
            con.Open();
            try
            {
                int result;
                SqlCommand cmd = new SqlCommand("spResetPassword", con);
                cmd.CommandType = CommandType.StoredProcedure;
                cmd.Parameters.AddWithValue("@NewPassword", credentials.newPassword);
                cmd.Parameters.AddWithValue("@Vercode", credentials.code);
                cmd.Parameters.AddWithValue("@UserEmail", credentials.email);
                cmd.Parameters.AddWithValue("@currentTime", DateTimeOffset.Now);
                cmd.Parameters.Add("@Return", SqlDbType.Int);
                cmd.Parameters["@Return"].Direction = ParameterDirection.Output;
                cmd.ExecuteNonQuery();
                result = (int)cmd.Parameters["@Return"].Value;
                return result;
            }
            catch (Exception ex)
            {
                Console.WriteLine("Exception is : " + ex.Message);
                return 0;
            }
            finally
            {
                con.Close();
            }
        }

        public (int, int) SendResetPasswordVerification(string email)
        {

            con.Open();
            try
            {
                Random r = new Random();
                int verificationCode = r.Next(1000000);
                int returncode;
                SqlCommand cmd = new SqlCommand("spResetPasswordVerification", con);
                cmd.CommandType = CommandType.StoredProcedure;
                cmd.Parameters.AddWithValue("@UserEmail", email);
                cmd.Parameters.Add("@Return", SqlDbType.Int);
                cmd.Parameters["@Return"].Direction = ParameterDirection.Output;
                cmd.Parameters.AddWithValue("@Vercode", verificationCode);
                cmd.Parameters.AddWithValue("@VercodeExpirationDttm", DateTimeOffset.Now.AddMinutes(15));
                cmd.ExecuteNonQuery();
                returncode = (int)cmd.Parameters["@Return"].Value;
                return (returncode, verificationCode);
            }
            catch (Exception ex)
            {
                Console.WriteLine("Exception is : " + ex.Message);
                return (0, 0);
            }
            finally
            {
                con.Close();
            }

        }

        private string Encrypt(string clearText)
        {
            string EncryptionKey = "MAKV2SPBNI99212";
            byte[] clearBytes = Encoding.Unicode.GetBytes(clearText);
            using (Aes encryptor = Aes.Create())
            {
                Rfc2898DeriveBytes pdb = new Rfc2898DeriveBytes(EncryptionKey, new byte[] { 0x49, 0x76, 0x61, 0x6e, 0x20, 0x4d, 0x65, 0x64, 0x76, 0x65, 0x64, 0x65, 0x76 });
                encryptor.Key = pdb.GetBytes(32);
                encryptor.IV = pdb.GetBytes(16);
                using (MemoryStream ms = new MemoryStream())
                {
                    using (CryptoStream cs = new CryptoStream(ms, encryptor.CreateEncryptor(), CryptoStreamMode.Write))
                    {
                        cs.Write(clearBytes, 0, clearBytes.Length);
                        cs.Close();
                    }
                    clearText = Convert.ToBase64String(ms.ToArray());
                }
            }
            return clearText;
        }

        private string Decrypt(string cipherText)
        {
            string EncryptionKey = "MAKV2SPBNI99212";
            byte[] cipherBytes = Convert.FromBase64String(cipherText);
            using (Aes encryptor = Aes.Create())
            {
                Rfc2898DeriveBytes pdb = new Rfc2898DeriveBytes(EncryptionKey, new byte[] { 0x49, 0x76, 0x61, 0x6e, 0x20, 0x4d, 0x65, 0x64, 0x76, 0x65, 0x64, 0x65, 0x76 });
                encryptor.Key = pdb.GetBytes(32);
                encryptor.IV = pdb.GetBytes(16);
                using (MemoryStream ms = new MemoryStream())
                {
                    using (CryptoStream cs = new CryptoStream(ms, encryptor.CreateDecryptor(), CryptoStreamMode.Write))
                    {
                        cs.Write(cipherBytes, 0, cipherBytes.Length);
                        cs.Close();
                    }
                    cipherText = Encoding.Unicode.GetString(ms.ToArray());
                }
            }
            return cipherText;
        }

        public int ActivateAccount(string emailId, string verificationCode)
        {
            con.Open();
            try
            {
                int result;
                SqlCommand cmd = new SqlCommand("spActivateAccount", con);
                cmd.CommandType = CommandType.StoredProcedure;
                cmd.Parameters.AddWithValue("@UserEmail", emailId);
                cmd.Parameters.AddWithValue("@VerCode", verificationCode);
                cmd.Parameters.AddWithValue("@currentTime", DateTimeOffset.Now);
                cmd.Parameters.Add("@Return", SqlDbType.Int);
                cmd.Parameters["@Return"].Direction = ParameterDirection.Output;
                cmd.ExecuteNonQuery();
                result = (int)cmd.Parameters["@Return"].Value;
                return result;
            }
            catch (Exception ex)
            {
                Console.WriteLine("Exception is : " + ex.Message);
                return 0;
            }
            finally
            {
                con.Close();
            }
        }

        public UsersInfo UsersList()
        {
            var userInfo = new UsersInfo();
            var users = new List<UserDetails>();
            var selectedServiceType = new List<ServiceTypeResponse>();
            con.Open();
            try
            {
                SqlDataReader dr;
                SqlCommand cmd = new SqlCommand("spListUsers", con);
                cmd.CommandType = CommandType.StoredProcedure;
                dr = cmd.ExecuteReader();
                while (dr.Read())
                {
                    var userDetail = new UserDetails
                    {
                        UserId = dr["UserId"] as int?,
                        displayName = dr["UserName"] as string,
                        email = dr["UserEmail"] as string,
                        UserRegDate = transformformat (dr["UserRegDate"] as string),
                        Level = dr["Level"] as string,
                        IsAdmin = dr["IsAdmin"] as string,
                        IsActivated = dr["IsActivated"] as string,
                        Location = dr["Location"] as string,
                        RackRate = dr["RackRate"] as decimal?,
                        CostRate = dr["CostRate"] as decimal?,
                        MobileNo = dr["MobileNo"] as string,
                        WorkSchedule = dr["WorkSchedule"] as int?,
                        BiodataType = dr["BiodataType"] as string,
                        BiodataContent = dr["BioConent"] as string,
                        ProfileImage = string.IsNullOrEmpty(dr["ProfileImage"].ToString()) ? DefaultProfile() : dr["ProfileImage"] as string
                    };
                    users.Add(userDetail); 
                }
                userInfo.userDetails = users;
                dr.NextResult();
                while (dr.Read())
                {
                    var serviceType = new ServiceTypeResponse()
                    {
                        UserId = dr["UserId"] as int?,
                        ServiceType = dr["ServiceType"] as string,
                        ServiceDescription = dr["ServiceDescription"] as string,
                    };
                    selectedServiceType.Add(serviceType);
                }
                userInfo.SelectedServiceTypes = selectedServiceType;
                return userInfo;
            }
            catch (Exception ex)
            {
                Console.WriteLine("Exception is : " + ex.Message);
                return null;
            }
            finally
            {
                con.Close();
            }
        }

        private string transformformat(string joiningDate)
        {
            DateTime date;
            if (DateTime.TryParseExact(joiningDate, "MMM d yyyy hh:mmtt", null, System.Globalization.DateTimeStyles.None, out date))
            {
                // Input date string matches the "MMM d yyyy hh:mmtt" format
                return date.ToString("yyyy-MM-dd");
            }
            else
            {
                return joiningDate;
            }
        }

        private List<ServiceTypeResponse> AreasOfExpertise(SqlDataReader dr)
        {
            var selectedServiceType = new List<ServiceTypeResponse>();
            while (dr.Read())
            {
                var serviceType = new ServiceTypeResponse()
                {
                    ServiceType = dr["ServiceType"] as string,
                    ServiceDescription = dr["ServiceDescription"] as string,
                };
                selectedServiceType.Add(serviceType);
            }
            return selectedServiceType;
        }

        public string DefaultProfile()
        {
            return "data:image/jpeg;base64,";
        }

        public string ManageUser(UserDetails userDetails)
        {
            con.Open();
            try
            {
                if (userDetails.FromAdmin.ToLower().Equals("true"))
                {

                    if (!string.IsNullOrEmpty(userDetails.ToDelete) && userDetails.ToDelete.ToLower().Equals("true"))
                    {
                        string userDetailsQuery = "Delete from Users where user_id = @user_id";
                        SqlCommand myCommand = new SqlCommand(userDetailsQuery, con);
                        myCommand.Parameters.AddWithValue("@user_id", userDetails.UserId);
                        myCommand.ExecuteNonQuery();
                    }
                    else
                    {
                        string userDetailsQuery = "Update Users Set user_reg_date = @user_reg_date, activated =@activated , is_admin =@is_admin , level =@level, profile_image = @profile_image where user_id = @user_id";
                        SqlCommand myCommand = new SqlCommand(userDetailsQuery, con);
                        myCommand.Parameters.AddWithValue("@user_reg_date", userDetails.UserRegDate);
                        myCommand.Parameters.AddWithValue("@activated", userDetails.IsActivated);
                        myCommand.Parameters.AddWithValue("@is_admin", userDetails.IsAdmin);
                        myCommand.Parameters.AddWithValue("@level", userDetails.Level);
                        myCommand.Parameters.AddWithValue("@user_id", userDetails.UserId);
                        myCommand.Parameters.AddWithValue("@profile_image", string.IsNullOrEmpty(userDetails.ProfileImage) ? string.Empty : userDetails.ProfileImage);
                        myCommand.ExecuteNonQuery();
                    }
                }
                else
                {
                    string userDetailsQuery = "Update Users Set reg_user_name = @reg_user_name, user_mail =@user_mail , mobile_no =@mobile_no , user_country =@user_country,work_schedule = @work_schedule, profile_image = @profile_image where user_id = @user_id";
                    SqlCommand myCommand = new SqlCommand(userDetailsQuery, con);
                    myCommand.Parameters.AddWithValue("@reg_user_name", userDetails.displayName);
                    myCommand.Parameters.AddWithValue("@user_mail", string.IsNullOrEmpty(userDetails.email) ? string.Empty : userDetails.email);
                    myCommand.Parameters.AddWithValue("@mobile_no", string.IsNullOrEmpty(userDetails.MobileNo) ? "0" : userDetails.MobileNo);
                    myCommand.Parameters.AddWithValue("@user_country", string.IsNullOrEmpty(userDetails.Location) ? string.Empty :  userDetails.Location);
                    myCommand.Parameters.AddWithValue("@work_schedule", userDetails.WorkSchedule ==null ? 0 :   userDetails.WorkSchedule);
                    myCommand.Parameters.AddWithValue("@user_id", userDetails.UserId);
                    myCommand.Parameters.AddWithValue("@profile_image",string.IsNullOrEmpty(userDetails.ProfileImage) ? string.Empty : userDetails.ProfileImage);
                    myCommand.ExecuteNonQuery();
                }

                return "Updated User details successfully";
            }
            catch (Exception ex)
            {
                Console.WriteLine("Exception is : " + ex.Message);
                return null;
            }
            finally
            {
                con.Close();
            }
        }
    }
}